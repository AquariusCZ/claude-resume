...
──────────────  飞书(可选,独立 Node 进程)  ──────────────
 ...
 feishu-agent.js ◀── WSClient 长连接 ──▶ 飞书开放平台
  ...
  · 会话状态机 {mode, project, sub, work} 存 feishu-sessions.json;按用户路由(userChats: open_id→私聊)
  · 事件 handler 毫秒级返回(ACK);claude 跑在后台 bg()+inflight(否则飞书停止投递=「白天卡死」)
  · 修改 = --resume <用户在会话列表里选的 id>;只读查询 = 隔离 cwd 的固定查询会话(首建时注入 AI_GUIDE.md)
  · 通知:lib.ps1 的 Send-FeishuNotify 经 config.feishuChatId(仅 owner 消息可绑定)推送,每条尾缀 · HH:mm
```
...
- **改代码后重新生效(关键,容易忘)**:线上从 `%LOCALAPPDATA%\ClaudeResume` 跑,**不会**自动同步 `src/`。
  ...
  - `feishu-agent.js`:复制到 AppDir → **杀 node**(`Get-Process node | Stop-Process -Force`)→ VBS 守护 ~8s 自动重启(重启会把全部飞书会话重置为 idle)→ 验证 `logs\feishu-stdout.log` 出现 `ws client ready`、node 进程为 1 个。
- **离线自测(不联网、不占单实例锁,改完先跑)**:
...
- **目录布局**:源码/文档在仓库(`C:\Users\23230\Desktop\claude-resume`);**运行态**全在 `%LOCALAPPDATA%\ClaudeResume\`:`config.json`、`state.json`、`logs\`、`node_modules\`、`feishu-agent.pid`、`checker.lock`、`feishu-token.json`(tenant token 缓存)、`feishu-sessions.json`、`feishu-query\`(每项目 `.started` 标记)、`feishu-query-cwd\<sha1>\`(只读查询的隔离 cwd)、`feishu-chat\`(闲聊 scratch 会话)。**密钥只在 `config.json`,gitignore,绝不进仓库。**

- **日志文件名解码**(都在 `logs\`,**一律本地时间**,绝不用 UTC)。样例 `run-20260716.log`:`run-` = 续跑引擎日志(`Write-CcuLog` 写,GUI 显示的就是它);`20260716` = 本地日期 yyyyMMdd,每天一个。GUI 永远读**最新的** `run-*.log`(`Get-CurLogFile` 按 `LastWriteTime`),绝不固定开窗那天的名字(跨午夜会空白)。其余:`feishu-<yyyy-MM-dd>.log` = 飞书 agent 日志(`logLine`);`feishu-stdout.log` = node stdout/stderr(SDK 连接日志,>2MB 由 `Clear-OldCaches` 清空);`gui-error.log` = GUI 自身异常。`Clear-OldCaches` 每 tick 还会删 >30 天的日报志、删 AppDir 探测残留会话。导出日志 = 合并所有 `run-*.log` + `gui-error.log` 成一个 **UTF-8 带 BOM** 文件。
...
- **项目发现与工作会话**:`~/.claude/projects/<encoded>/*.jsonl`,文件夹名对路径有损/歧义,所以读每个 session `*.jsonl` 头部真实 `cwd`(`-Encoding UTF8`,否则中文路径丢)+ 最后使用时间。✏️修改的会话列表 `listProjectSessions` 同样按 cwd 定位文件夹,标题取 jsonl 顶部的 `ai-title`(约第 8 行,**只读文件头**,27MB 转录也不整读);摘要 `sessionPreview` 只读**文件尾** N 字节。

- **JSON 内部结构**(完整字段见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)):
  - `config.json`(GUI 写,UTF-8 **无 BOM**——有 BOM 会崩 Node 的 JSON.parse):`enabled`(布防总开关)、`selected`/`customProjects`(`{name,path}`)、`resumePrompt`、`skipPermissions`、`dirtyGuard`(`stash`|`branch`)、`probeIntervalMinutes`(5/15/30,限流时收紧到 4)、`probeModel`/`resumeModel`、`continuous`、`feishuAppId/AppSecret/ChatId/Webhook/Secret`、`feishuChatModel`(闲聊+项目+查询共用,GUI 模型 chip 同步)、`feishuAuthOpenIds`(**owner** 名单)、`feishuAuthPassword`(「解锁 <密码>」自助授权)。`feishuViewerOpenIds` 已是遗留 no-op。
...
- **Q:✏️修改项目会续到哪个会话?会不会猜错?** — A:**不猜**。点 ✏️修改先弹**会话选择卡**:该项目最近 5 个 claude 会话(claude 自动标题 + 最后使用时间)或 🆕 新开会话;选中后先发**最近 2 轮摘要**让你想起停在哪,之后每条消息 `--resume <那个 id>`;🔀 切换会话随时重选。没选之前发指令只会重新弹选择卡。
- **Q:同事用机器人,消息会不会串到我这里/他能看到我的通知吗?** — A:不会。每个用户的菜单/回复路由到**他自己与机器人的私聊**(`userChats`/`userTarget`);只有 **owner 本人的消息**才能绑定/改变通知聊天 `feishuChatId`,同事的点击/消息**绝不劫持**它。回归测试 `test/routing.js`。
...
| 更新导览 (project tour) | GUI 按钮/`Invoke-ProjectTour`/`project-tour` skill:生成本 AI_GUIDE.md 供查询提速提准 |

## 8. 文档索引(深挖时读)
- [README.md](README.md):安装、使用、飞书配置(开发者后台 3 处配置 + 发布版本)、两类项目会话、权限模型——面向用户的完整说明。
