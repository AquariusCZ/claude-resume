<!-- project-tour · generated 2026-07-16 22:33 · git e87ba32 -->
...
──────────────  飞书(可选,独立 Node 进程)  ──────────────
 ...
 feishu-agent.js ◀── WSClient 长连接 ──▶ 飞书开放平台
  ...
  · 回复按用户路由(userChats: open_id→各自私聊,仅 p2p 学习);通知经 Send-FeishuNotify → feishuChatId(仅 owner 可绑定),每条尾缀 · HH:mm
```
...
```
   ...
   (scratch 会话,非 owner            ├─ sub=query  → 隔离 cwd 的固定查询会话(首建注入 AI_GUIDE.md)
    降权:plan+禁文件/执行)          └─ sub=modify → 先弹会话选择卡(owner-only,选 work id)→ --resume <work>
...
| `src/checker.ps1` | **无状态状态机**,计划任务每 ~2 分钟跑一次;文件锁防重入;节流→探测→限流/恢复→FIRE 续跑;误判限流 ≥6 次熔断;`-DryRun` 预演。真正的开关是 `config.enabled`。 | 顶层线性脚本(无函数):节流(`sawLimited`→4min,否则 `probeIntervalMinutes`)、fail-closed、一次性(成功后 `enabled=false`) |
...
| `src/feishu-agent.js` | 飞书双向 agent(Node 长连接):消息/按钮 → 会话状态机(闲聊/只读查询/修改+会话选择)+ 权限(owner/viewer)→ 后台跑 claude → 按用户路由回复。离线测试模式 `FEISHU_TEST` / `FEISHU_TEST_NO_CLAUDE`。 | `onMessage`/`onCardAction`/`onBotMenu`、`bg`+`inflight`(后台执行,handler 秒回)、`killTree`(taskkill /T /F 杀整棵进程树)、`runClaude`(prompt 走 **stdin**)、`runProjectQuery`(注入 AI_GUIDE + `projectGitHash` 新鲜度)、`listProjectSessions`/`sessionPreview`/`buildSessionCard`(✏️修改的会话选择器)、`buildMenuCard`/`buildProjectCard`、`showCard`/`refreshCard`/`currentCard`、`userTarget`/`rememberUserChat`(按用户路由,od: 迁移)、`authLevel`、`querySession`/`clearQuerySession`、`MODELS` 注册表/`modelLabelOf`、`apiRetry`、`startHeartbeat` |
| `src/install.ps1` | 部署:src→AppDir、生成珊瑚色图标、桌面快捷方式、注册计划任务、飞书 Startup 项 + `npm install`。**可反复跑**(=重新部署),末尾把 `enabled/armed` 置 false(未布防)。 | 顶层脚本 |
| `src/*.vbs` | 隐藏启动器:`launcher.vbs`(GUI)、`checker-launch.vbs`(计划任务)、`feishu-launch.vbs`(守护 node,自动重启+重定向 stdout,>1MB 轮转)。 | wscript 隐藏窗口 |
| `test/*.js` | 8 个自测(3 离线 + 5 跑真 claude,见 §4)。 | `card-flow`/`routing`/`session-pick`/`concurrency`/`modify-resume`/`query-e2e`/`chat-security`/`guide-e2e` |
...
## 4. 测试 / 运行流程
- **安装/部署入口**:`powershell -ExecutionPolicy Bypass -File src\install.ps1`。复制到 AppDir、建桌面快捷方式「Claude续跑」、注册计划任务 `ClaudeResumeChecker`(每 2 分钟),初始**未布防**。
...
- **离线自测(不联网、不占单实例锁、不跑真 claude,改完先跑,别每次去飞书点)**:
  - `node test/card-flow.js` — 卡片/菜单状态机:「进项目→积压菜单事件」不堆卡、不跳回主菜单。
  - `node test/routing.js` — 三组真实回归(设 `FEISHU_TEST_NO_CLAUDE=1`,**绝不 spawn 真 claude**):A) 同事的菜单/卡片回复进**同事自己**的私聊、绝不劫持 `feishuChatId`,群里 @ 机器人**不污染** p2p 映射;B) 会话内的「选 A」「1」归会话、不被当命令(idle 下仍是命令);C) 模型注册表(Fable 5 按钮、自由 `模型 claude-*`、垃圾拒绝)。
  - `node test/session-pick.js` — ✏️修改必先弹会话列表(带最近 2 轮摘要)、没选之前**绝不跑 claude**、🔀切换/🆕新开正常。
- **跑真 claude 的自测**(mock 飞书 API,haiku,~$0.1 级):
  - `node test/concurrency.js` — 「白天卡死」回归:查询 handler 秒回,查询进行中菜单/卡片仍即时响应,二次查询得「查询进行中」,原查询仍送达。
...
- **续跑(fire)路径**:依赖真实额度,无法离线完全验证。先按 **预演(Preview / `checker -DryRun`)** 看计划,真实重置自行验证。
...
- **目录布局**:源码/文档在仓库(`C:\Users\23230\Desktop\claude-resume`);**运行态**全在 AppDir(`%LOCALAPPDATA%\ClaudeResume\`):
  - `config.json`(GUI 写)/ `state.json`(checker 写)/ `feishu-sessions.json`(agent 写)/ `feishu-userchats.json`(open_id→私聊映射);
...
- **日志文件名解码**(都在 `logs\`,**一律本地时间**,绝不用 UTC)。真实样例 `run-20260716.log` 拆解:
  ...
  - 其余:`feishu-<yyyy-MM-dd>.log` = 飞书 agent 日志(`logLine`);`feishu-stdout.log` = node stdout/stderr(SDK 连接日志,>1MB 由守护 vbs 轮转、>2MB 由 `Clear-OldCaches` 清空);`gui-error.log` = GUI 自身异常。
...
- **JSON 内部结构**(完整字段见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)):
  - `config.json`(GUI 写,UTF-8 **无 BOM**——有 BOM 会崩 Node 的 JSON.parse)核心字段:
    ...
    ```jsonc
    {
      "enabled": false,             // 布防总开关(解除=立即停一切;checker 只认这个)
    ...
    ```
    (`feishuViewerOpenIds`/`feishuAllowOpenIds` 多为遗留/白名单 no-op——非 owner 自动就是 viewer。)
  ...
  - `feishu-sessions.json`:chatId(或未建映射时的 `od:<open_id>` 伪目标)→ `{mode: idle|chat|project, project, sub: query|modify, work}`;`work` = ✏️修改要续的 claude 会话 id(用户从列表选的,或 🆕 新开的 uuid);**未选时不跑任何东西**;agent 每次(重)启动全部重置为 idle。
...
- **Q:改了 `src/feishu-agent.js` 为什么没反应?** — A:线上从 AppDir 跑。必须复制到 AppDir → 杀 node → 等 ~8s 守护重启 → 看 `feishu-stdout.log` 的 `ws client ready`。见 §4 / `CLAUDE.md`。
...
- **Q:点「停止」为什么以前停不掉、还把活干完了?** — A:老代码 `child.kill()` 在 Windows 上只杀了 `cmd.exe` 外壳,真正干活的孙进程(claude→node)还在跑,曾出现「已停止」的跑仍完成并提交。现在用 `killTree`(`taskkill /pid <pid> /t /f`)**连根拔起整棵进程树**,超时和「停止」都走它;`停止` 还覆盖该项目的**修改跑、查询跑(隔离 cwd)和闲聊跑**三种。
- **Q:✏️修改项目会续到哪个会话?会不会猜错?** — A:**不猜**。点 ✏️修改先弹**会话选择卡**(owner 专属):该项目最近 5 个 claude 会话(claude 自动标题 + 最后使用时间)或 🆕 新开会话;选中后先发**最近 2 轮摘要**让你想起停在哪,之后每条消息 `--resume <那个 id>`;🔀 切换会话随时重选。没选之前发指令只会重新弹选择卡(`test/session-pick.js` / `test/modify-resume.js` 验证)。viewer 连会话选择卡都进不去。
- **Q:同事用机器人,消息会不会串到我这里/他能看到我的通知吗?群里 @ 机器人会乱吗?** — A:不会。每个用户的菜单/回复路由到**他自己与机器人的私聊**(`userChats`/`userTarget`),而且 `userChats` **只从 p2p 私聊消息学习**——群里 @ 机器人不会污染他的映射;卡片点击事件不带 chat_type,所以点击**既不重绑通知聊天也不改映射**(只在完全没映射时兜底填一次)。只有 **owner 本人的 p2p 消息**才能绑定通知聊天 `feishuChatId`,且发送者必须**明确在** `feishuAuthOpenIds` 里(未锁定+尚无绑定时才有首条 p2p 兜底绑定)。同事若在你之前点过底部菜单,他的状态先落在 `od:<open_id>` 伪目标,首次发消息时自动迁移到真实私聊(`rememberUserChat`)。`test/routing.js` 覆盖全部这些。
- **Q:谁能改我的项目?怎么控权限?** — A:只有 `config.json` 的 `feishuAuthOpenIds`(**owner/full**)能改项目/改配置/授权/停止运行/浏览会话选择卡;**其他所有人自动只读 viewer**(浏览/查询),进项目直接进只读问答,无死按钮。空名单 = 未锁定 = 人人可改,移除最后一个 owner 会解锁,GUI 会警告。闲聊对所有人开放,但**非 owner 的闲聊是 plan 模式且禁文件/执行工具**(套不出 config.json 密钥,`test/chat-security.js` 验证)。
- **Q:「只读查询」会不会改到代码 / 污染我 VS Code 的会话?** — A:不会。查询跑 `--permission-mode plan`,在**隔离 cwd**(`feishu-query-cwd\<sha1>`)+ `--add-dir` 里,固定 session-id,`--disallowedTools Task`。与 ✏️修改(`--resume` 你选的工作会话)完全隔离。注意 `查询/只读` 前缀**必须带空格或冒号分隔**(如 `查询 xxx`),否则修改会话里以「只读」开头的正常话不会被误路由进查询会话。
- **Q:在会话里回答 claude 的「选 A」为什么以前会跳回菜单?** — A:旧版在任何模式下都做模糊命令匹配,「选 A」「1」、问候语、甚至「解锁 …」都被劫持成命令。现在**模糊匹配/解锁/密码只在 idle 模式**生效;会话内自由文本一律归会话。显式命令(退出/菜单/停止/模型/帮助)任何模式仍可用。
- **Q:想用新出的模型怎么办?要改代码吗?** — A:不用。按钮注册表 `MODELS` 覆盖当前一线(**默认 / Fable 5 / Opus / Sonnet / Haiku**,当前领衔按钮是 Fable 5=`claude-fable-5`),文本命令 **`模型 <任意 claude-* 完整 id>`** 接受任何合法 id,新模型发布当天即可用;GUI 模型 chip 与飞书共享 `feishuChatModel`。
...
- **Q:日志在哪、怎么看?** — A:AppDir 的 `logs\`,见 §5。GUI 日志区(可「⤢ 弹出大窗」)显示最新 `run-*.log`;飞书问题看 `feishu-<date>.log` 和 `feishu-stdout.log`;「导出日志」合并成一份可分享文件。通知每条尾部的 `· HH:mm` 是事件真实发生的本地时间。
...
- **Q:飞书里有哪些文本命令?**(与按钮等价;标 ⌂ 的仅在主菜单/空闲状态生效,进入会话后你打的字都属于会话)— A:
  - 导航:`菜单`/`项目`(主菜单卡,任何时候可用)、⌂`进入 <编号|名字>`、⌂直接发项目名/编号、`退出`(任何时候可用);
  - 干活:`查询 <问题>`(只读直达,带分隔符)、⌂`<项目名> <指令>`(一次性)、`停止`(owner,杀整棵树);
...
| 修改项目 (modify) | ✏️:先从**会话选择卡**(owner-only)挑一个 claude 会话(或 🆕 新开),之后 `--resume <该 id>` 续它 |
...
| owner / viewer | owner=在 `feishuAuthOpenIds`,能改项目+配置+浏览会话卡;viewer=其他所有人,自动只读 |
| 按用户路由 (per-user routing) | `userChats`(open_id→私聊,**仅 p2p 学习**)+ `userTarget`:每人的回复进自己私聊;`feishuChatId` 只认 owner 的 p2p |
| od: 伪目标 | 用户在建映射前(如先点了底部菜单)状态暂存于 `od:<open_id>`;首条 p2p 消息迁移到真实私聊 |
...
| killTree | `taskkill /pid <pid> /t /f`:杀 `cmd→claude→node` 整棵进程树(`child.kill()` 只杀 cmd 外壳,孙进程会跑完) |
...
| 模型注册表 (model registry) | `MODELS` 按钮一线模型(默认/Fable 5/Opus/Sonnet/Haiku) + 「模型 claude-*」自由 id,新模型零改码可用 |
...
- [docs/LESSONS.md](docs/LESSONS.md):真实踩坑 + 开发史(PS 5.1 exit-code/BOM/时区、cmd 换行截断 `-p`、`--continue` 池污染、飞书卡片堆叠/抢卡、进程树 kill、**测试绝不能对真实项目跑修改会话**的墨菲现场、权限模型简化、离线自测方法论)。排查「为什么这么写/为什么会这个 bug」看它。
