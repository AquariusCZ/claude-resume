<#
  ...
  Cost control: probes use the configured fixed cadence, tightening to ~4 min once limited;
  a rate-limited probe is rejected server-side so it doesn't consume quota.
...
try {
  ...
  if(-not (Recover-CcuBackgroundChild)){ return }
  $cfg = Get-CcuConfig
  ...
  $cycleId = if($DryRun){ "$($cfg.armCycleId)" } else { Get-OrCreate-CcuActiveCycle }
  if(-not $DryRun -and -not $cycleId){ return }
  function Test-ThisCycle { return [bool](Test-CcuCycleActive $cycleId) }
  function Save-ThisCycleState($Value) {
    if($DryRun){ return $true }
    if(-not (Test-ThisCycle)){ return $false }
    $Value.cycleId=$cycleId
    return [bool](Set-CcuState $Value)
  }

  $state = Get-CcuState
  if(-not $DryRun -and "$($state.cycleId)" -ne $cycleId){
    if(-not (Initialize-CcuCycleState $state $cycleId)){ return }
  }
  ...
  if($DryRun){
    ...
    Write-CcuLog ("DRY-RUN: 布防后策略 = 每 ${ivl}m 实探一次额度(限流后加密到 4m), 恢复瞬间无总时限续跑这 $($selected.Count) 个项目: $names") 'info'
    return
  }
  ...
  if($sinceProbe -lt $minGapMin){
    $state.phase='waiting'; $state.projectStatus=$pstat; if(-not (Save-ThisCycleState $state)){ return }
    if(-not $sawLimited){
  ...
  $state.lastProbeUtc = $nowU.ToString('o'); if(-not (Save-ThisCycleState $state)){ return }
  ...
  if($probe.reason -eq 'limited'){
    ...
    $state.sawLimited=$true; $state.phase='waiting'; $state.projectStatus=$pstat; if(-not (Save-ThisCycleState $state)){ return }
  ...
  $state.phase='resuming'; $state.projectStatus=$pstat; if(-not (Save-ThisCycleState $state)){ return }
  Write-CcuLog '额度已恢复 -> 开始逐个续跑' 'ok'
  [void](Send-FeishuNotify ('额度恢复,开始续跑 ' + $selected.Count + ' 个项目'))
  $cycleCancelled = $false
  foreach($sel in $selected){
    if(-not (Test-ThisCycle)){ Write-CcuLog '布防轮次已变化 -> 停止后续项目' 'info'; $cycleCancelled=$true; break }
    ...
    if(-not (Test-ThisCycle)){ Write-CcuLog 'git-guard 后检测到解除/重新布防 -> 不启动 AI' 'info'; $cycleCancelled=$true; break }
    Write-CcuLog ('续跑(无总时限) -> ' + $sel.name + '  [' + $sel.path + ']') 'launch'
    $cancelWhenCycleEnds = { return (-not [bool](Test-CcuCycleActive $cycleId)) }
    $r = Invoke-ClaudeResume -Project $sel -Prompt $cfg.resumePrompt `
           -SkipPermissions:([bool]$cfg.skipPermissions) -Model $cfg.resumeModel `
           -CancelFlag $cancelWhenCycleEnds -RunKey ($cycleId + '|' + "$($sel.path)")
    if(-not (Test-ThisCycle)){
      Write-CcuLog ('当前续跑已随解除/重新布防停止: ' + $sel.name + ' (' + $r.status + ')') 'info'
      $cycleCancelled=$true
      if($r.status -match 'stop-failed|registry'){ [void](Send-FeishuNotify ('⚠️ ' + $sel.name + ' 停止未确认,已锁定后台续跑等待回收')) }
      break
    }
    $pstat[$sel.path] = $r.status; $state.projectStatus=$pstat; if(-not (Save-ThisCycleState $state)){ return }
    ...
    if($r.status -match 'stop-failed|registry'){ Write-CcuLog '后台进程状态未确认 -> 保留登记并停止本轮' 'error'; return }
    if($r.status -eq 'limited'){
      ...
      if($state.limitedRefires -ge 6){
        ...
        $pstat[$sel.path]='error'; $state.projectStatus=$pstat; if(-not (Save-ThisCycleState $state)){ return }
        continue
      }
      Write-CcuLog '续跑中又被限流 -> 停,回到等待' 'warn'
      [void](Send-FeishuNotify ($sel.name + ' 续跑中又限流,等待重置后重试'))
      $state.sawLimited=$true; $state.phase='waiting'; [void](Save-ThisCycleState $state); return
    }
  }
  if($cycleCancelled -or -not (Test-ThisCycle)){ return }
  $state.phase='done'; $state.sawLimited=$false; $state.limitedRefires=0; $state.projectStatus=$pstat
  if(-not (Save-ThisCycleState $state)){ return }
  $completion = Complete-CcuCycle $cycleId
  if($completion -eq 'disarmed'){ Write-CcuLog 'checker: 一次性完成 -> 已解除' 'ok'; [void](Send-FeishuNotify '🎉 全部完成,已解除布防') }
  elseif($completion -eq 'continuous'){ Write-CcuLog 'checker: 续跑完成(连续模式)' 'ok'; [void](Send-FeishuNotify '本轮完成(连续模式)') }
}
finally {
