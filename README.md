...
**Two kinds of project session.** **✏️ 修改项目** runs `claude --continue` and continues the **same** conversation your VS Code session shows (reopen the session to see it — the panel doesn't live-refresh external appends). **👁 只读查询** runs `--permission-mode plan` (reads code / answers, never edits) in **one dedicated per-project query session** (a fixed session id) — every read-only query for a project, from anyone at any time, builds up that one shared Q&A, separate from your work session. Clear it from the card's **[🧹 清空查询记忆]**, the **忘记查询** command, or the **清查询** link on that project's card in the GUI.

**Authorization (3 levels).** Chat is open to everyone; touching projects is gated. `feishuAuthOpenIds` empty = not locked (anyone can do anything). Once you add yourself, others are **none** (chat only) until you grant them — when an unauthorized user messages the bot you get a card with **[✅ 可改项目]** / **[👁 只读查询]** buttons, or type **`授权 ou_xxx`** (full: modify) / **`只读授权 ou_xxx`** (viewer: read-only queries only). A viewer's every project message is forced read-only; only **full** users can modify. Manage with **取消授权** / **取消只读** / **授权列表**. (A user's `ou_…` open_id is shown to them the first time they message the bot.) The Feishu developer console does **not** show this list — it only has 应用可用范围 (who may use the app at all); the real full/viewer roster lives in `config.json` and is viewable/removable in the GUI's **授权用户** window.

The GUI mirrors the agent's state (all via the same `config.json`): a **模型 chip** (top-right, click to cycle 默认/Sonnet/Opus/Haiku), bottom-row **忘记闲聊** / **授权用户** buttons, and a per-project **清查询** link on each project card. Feishu-side, all menu navigation (main menu ⇄ project sub-menu) updates **one control card in place** rather than sending a new card each time.
