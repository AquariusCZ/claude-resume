...
> **历史 CLI 降级设计,不再是现役契约。** 本文冻结于 2026-08-06,用于解释 Windows 上为何保留 `ClaudeCodeProbe` 及其首版解析器。当前数据源优先级、OAuth 协议、稀疏合并、持久化和 UI 语义以 [`../CLAUDE-QUOTA-ACQUISITION.md`](../CLAUDE-QUOTA-ACQUISITION.md) 为准;本文与现役手册或代码冲突时,不得按本文回退实现。
