...
A scheduled task would be better still — S4U keeps the process off the interactive desktop and adds restart-on-failure — but registering one requires elevation (non-elevated registration returns `0x80070005` for every root/subfolder × S4U/Interactive combination), and `install` is not elevated. `scripts/register-autostart.ps1` performs that upgrade from an elevated shell: it registers the task, reads it back and verifies eleven contract properties before reporting success, then removes the Startup shortcut. `install` detects an existing task by probing `%WINDIR%\System32\Tasks\AI Resume 续跑引擎` (readable without elevation) and skips creating the shortcut, so the two paths never both launch a Worker.

The C# suite currently contains 1161 xUnit tests. It uses isolated temporary state, synthetic sessions and injected process/API runners. It does not start an AI modify run against a real project or send a paid API request.
