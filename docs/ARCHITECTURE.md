...
If the OAuth route cannot provide a usable snapshot, `ClaudeCodeProbe` remains a diagnostic/UI fallback. It cannot authorize an automatic resume: production wires `ResumeEngine` through the singleton `QuotaService` and `QuotaResumeProbe`. Arming requires an explicit Fable, Opus, Sonnet or Haiku target. Only a fresh OAuth snapshot with current five-hour, seven-day and a `weekly_scoped` utilization matching that target can return ready. The admitted model is carried with the evidence and passed to the runner as the same `--model`; a model change during probing, between projects or at the final pre-spawn gate invalidates the evidence and forces a refresh. Accounts whose responses never include the matching scoped window remain waiting by design; five-hour/seven-day availability is not evidence that the target model can run. The resume flow is:

```text
user selects target model and arms projects
        |
        v
ResumeEngine observes usage/limit state
        ...
        |
        +-- limited -----> persist cycle and wait
                              |
                              v
                  force-refresh complete quota evidence
                              |
                  +-----------+-----------+
                  |                       |
             any blocked/unknown   fresh OAuth + matching scope
                  |                       |
                  +--> keep waiting       v
                                  resume first project
                                          |
                                  force-refresh before
                                  every later project
                                          |
                                          v
...
Project-row status is scoped to the currently active arm cycle. A one-shot cycle keeps its final result in SQLite for diagnostics but stops exposing it as current UI state as soon as the Worker auto-disarms; continuous mode remains armed and may keep showing the current cycle's progress. `arm.get` holds `config.json.write.lock` while capturing both config and strict SQLite state, so it cannot observe the intermediate commit where cycle state is already done but one-shot disarm has not yet reached config. Manual refresh, the 30-second poll, arm and disarm all re-read and repaint both the engine summary and every project row. Known local failures and unknown status codes are classified as failures with Chinese UI text instead of being rendered as neutral raw codes.

`ActiveRunId` and `PendingCancellationRunId` are cross-cycle safety gates. A matched or unverifiable owned process blocks every new resume attempt; cleanup occurs only after the existing `RunId` is proven gone or mismatched. The fresh quota evidence that triggered `StartResuming` admits only the first project. After any project runs, `ResumeEngine` force-refreshes `QuotaService` before the next spawn and rechecks the arm cycle and current selection after that asynchronous boundary. Any scoped limit, CLI-only evidence, carried value, unavailable snapshot or probe exception returns the cycle to waiting without starting the next process. Newly selected projects join the current round in order, and the completion commit rechecks the latest queue under the config lock before marking the cycle done. Disarm or removal requests cancellation by exact `RunId`, and an unconfirmed stop preserves the gate. Resume output treats only a structured `rate_limit_event.rate_limit_info.status` as retryable quota evidence. A rate-limit event after Write/Edit/Bash, unknown tool activity, malformed NDJSON or a non-object NDJSON line becomes `limited-side-effects` and blocks the cycle, while explicitly read-only tools may still return to quota waiting. A recovered one-shot `phase=done + armed=true` state only completes the pending disarm and never probes or replays the round. Schema v6 seeds the singleton `product_state` row, and the Worker uses strict reads that fail the whole tick closed on missing-row, SQLite or JSON errors. The control plane also treats those failures as `StateUnverified`, clears stale project rows and avoids any positive armed-state claim. A bare `ActiveRunId` that disappears without cancellation evidence becomes `exit-null` (unconfirmed completion); only a confirmed `PendingCancellationRunId` exit becomes `stopped`. The control plane explicitly reports a matched old-cycle run as still active and an unverifiable one as unconfirmed instead of presenting a newly armed cycle as merely awaiting initialization. Reconciliation still runs while disarmed, and a missing Worker remains the highest-priority armed-state warning.

Before IPC, observation and resume services start, the Worker calls the existing `RecoverAsync` pass over stale process registrations; hosted-service startup is explicitly forced to remain serial so configuration cannot bypass that gate. A recovered `starting` run whose registration still matches is advanced to `running` without another spawn; unverifiable evidence keeps it in `starting` and preserves the run key. Process supervision keeps the Windows Job Object handle until `QueryInformationJobObject` reports `ActiveProcesses=0`. While the current process owns that exact Job, both the orchestrator and `ResumeEngine` use the supervisor status as their authority instead of reading only the registry's outer PID; an exited wrapper with live descendants is therefore normal `Alive` work rather than a monitor failure, and cancellation uses `TerminateJobObject` before releasing the handle. The GUI has no Job handle, so an existing registration whose outer PID disappeared is reported as unverified rather than finished. The current dependency graph contains no equivalent Job wrapper, so this remains a thin `kernel32` P/Invoke extension of the existing implementation. `Process.Start` cannot create suspended processes, leaving a documented narrow Start-to-Assign crash window; assignment failure is mitigated by persisting the exact PID first, requesting cancellation immediately and retaining ownership until termination is confirmed instead of deleting the registry row. Every consumer of `ProcessStartResult` treats `Started=true` with a non-empty error code as an in-doubt launch that must be cancelled before it can become `running`.

For resume startup, an `OperationCanceledException` from `IProcessSupervisor.StartAsync` has the narrower contract that no registry entry or process-creation side effect has begun. `ClaudeResumeRunner` returns that fact together with the prepared `RunId`; `ResumeEngine` removes the pre-spawn `running` marker only when cycle, project, status and exact `RunId` still match. Any mismatch remains fail-closed. Ordinary launch failures and in-doubt starts never use this rollback path.

The RunContract start driver is also the per-`RunId` start/cancel serialization boundary. Cancel is persisted before waiting for an active driver; the driver re-reads it after provider startup, after recovered-process inspection and after supervisor startup, so a run cannot become terminal-cancelled and then spawn an unowned process. Unconfirmed cancellation is retried by exact `RunId` on every observation. A provider failure is persisted on the still-running row while process-tree termination is pending; only `ActiveProcesses=0` (or equivalent exact gone evidence) permits terminal settlement and run-key release. Settlement reads the cancellation marker and pending failure and writes the selected terminal state in one SQLite `IMMEDIATE` transaction, with a committed user cancellation taking priority over failure.
...
The C# suite currently contains 1386 xUnit tests. It uses isolated temporary state, synthetic sessions and injected process/API runners. It does not start an AI modify run against a real project or send a paid API request.
