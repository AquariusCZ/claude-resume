...
process.env.FEISHU_TEST_AI_DELAY_MS = '1800';
...
const testConfig = testConfigHelper.prepareTestConfig({
  ...
  source: {
    enabled: true,
    feishuChatId: 'oc_concurrency_test',
    feishuAuthOpenIds: ['ou_concurrency_owner'],
...
async function main() {
  {
    const PROJ = A.discoverProjects().find(p => /Sample-Analyzer/i.test(p.name)) || A.discoverProjects()[0];
