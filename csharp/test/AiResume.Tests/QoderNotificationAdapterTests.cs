...
/// <summary>
/// QoderNotificationAdapter 的单元测试。
/// 所有测试均在系统临时目录下创建唯一子目录,避免触碰真实用户配置。
/// </summary>
public class QoderNotificationAdapterTests : IDisposable
{
    private readonly string _tempDir;
    private readonly string _settingsPath;
    private readonly QoderNotificationAdapter _adapter;
    ...
    /// <summary>
    /// 必须用**生产真正会传的命令形状**(安装层写的是 …\AiResume.Hook.exe)。
    ///
    /// 这里原本是 <c>C:\tools\airesume-completion-hook.cmd</c> —— 它恰好等于当时的
    /// MarkerFileName,于是「写完认得出自己」这条**在测试里恒成立、在生产里恒不成立**:
    /// 生产传的是 AiResume.Hook.exe,与标记对不上,探测永远报未安装、
    /// 每次启用都再追加一条。实测用户配置里堆了 14 条重复钩子,而测试一直是绿的。
    ///
    ...
    public void Disable_RemovesOnlyOwnEntries()
    {
        ...
        Assert.DoesNotContain(QoderNotificationAdapter.MarkerFileName, cmd.GetString(), StringComparison.OrdinalIgnoreCase);
    }
