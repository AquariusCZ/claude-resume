...
- **自动续跑的唯一额度入口是 `QuotaService` + `QuotaResumeProbe`。** 生产 DI 禁止把 `ResumeEngine` 直接接到 `ClaudeCodeProbe`:CLI/Haiku 的成功或限流都只证明该探针模型,不能证明 Fable 等目标模型额度状态。布防必须显式选择 Fable/Opus/Sonnet/Haiku 之一并写入 `ResumeModel`;只有本账号实时 OAuth 快照包含与该目标模型族匹配的本次 `weekly_scoped`、所有主窗口与返回的 scoped 窗口均未满、且没有 `carriedForward` 历史承接时才可放行。探测证据、配置模型与 runner 的同一个 `--model` 必须绑定;任一异步边界发现模型变化都丢弃旧证据并重新取证,spawn 前再精确复核。OAuth/存储/网络不可验证、目标 scoped 缺失、模型未配置或 CLI 降级一律等待。该规则安全优先:服务端始终不返回目标 scoped 窗口的账号不会自动续跑,也不能用 5H/7D 正常猜测原会话模型可用。5H reset 只表示可以重新取证,绝不单独触发续跑。首项目可复用刚触发恢复的证据,此后每个项目 spawn 前必须强刷并重新过闸;前一项目可能已经耗尽 scoped 额度。`limited-side-effects` 仍锁住本周期,不得因额度恢复自动重放。
